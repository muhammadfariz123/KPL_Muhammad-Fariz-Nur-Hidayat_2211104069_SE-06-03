...
function parseAngkaKetat(teks, lokal) {
  ...
  if (lokal === 'en-US') {
    ...
    regex = /^\$?(\d{1,3}(,\d{3})*|\d+)(\.\d+)?$/;
    if (regex.test(teks)) {
      // Hapus koma dan simbol lainnya, simpan titik desimal
  ...
  if (lokal === 'fr-FR') {
    // Format fr-FR: spasi ribuan, koma desimal, opsional euro di akhir
    regex = /^(\d{1,3}( \d{3})*|\d+)(,\d+)?\s?€?$/;
    if (regex.test(teks)) {
      // Hapus spasi ribuan, ubah koma ke titik, hapus karakter non-digit/non-dot
